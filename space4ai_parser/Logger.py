...
class Logger:
    ...
    ## @var verbose
    # Verbosity level (default: 0 - minimal logging)
    ...
    def log(self, message, v = 0):
        if self.verbose >= v:
            ...
            print(
                f"{datetime.now()}: [SPACE4AI-Parser] {full_message}", 
                file = self.stream, 
                flush = True
            )
    ...
    def warn(self, message):
        full_message = f"[SPACE4AI-Parser] WARNING: {message}"
        print(full_message, file = self.stream, flush = True)
